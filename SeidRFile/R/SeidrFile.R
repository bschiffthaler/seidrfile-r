...
setGeneric("centrality", function(object) standardGeneric("centrality"))
...
setMethod("show", signature="SeidrFile", function(object) {
  ...
  cat("  Centrality:         ", object@sf_attr$ncentrality, "\n")
})
...
#' @export
setMethod("centrality", signature="SeidrFile", function(object) {
  vdata <- SeidrFileHeader__ptr__getCentrality(object@sf_h_ptr)
  vnames <- SeidrFileHeader__ptr__getCentralityNames(object@sf_h_ptr)
  r <- split(vdata, rep(seq(1, object@sf_attr$ncentrality), each=object@sf_attr$nodes))
  names(r) <- vnames
  return(r)
})
