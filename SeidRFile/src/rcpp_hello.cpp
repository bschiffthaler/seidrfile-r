...
List SeidrFileHeader__ptr__getAttr(SEXP xp) {
  ...
  uint16_t ncentrality = ptr->attr.pagerank_calc;
...
// [[Rcpp::export]]
NumericVector SeidrFileHeader__ptr__getCentrality(SEXP xp) {
  XPtr<SeidrFileHeader> ptr(xp);
  return wrap(ptr->centrality_data);
}
// [[Rcpp::export]]
CharacterVector SeidrFileHeader__ptr__getCentralityNames(SEXP xp) {
  XPtr<SeidrFileHeader> ptr(xp);
  return wrap(ptr->centrality_names);
}
